...
Original file is located at
    https://colab.research.google.com/drive/14OT7ZA7eZmEOZJ1hZRK3t4pau_4krPbK

Cell 1 — Install dependencies
"""

!pip install google-api-python-client tenacity --quiet

"""Cell 2 — Save the extractor file to Colab"""
...
"""Cell 3 — Test it works

"""

# Make sure Python loads the fresh version, not a stale cached one
import importlib
import extractor
...
from extractor import (
    parse_video_id, get_client, fetch_video_meta, fetch_channel_info,
    fetch_comments, engagement_rate, parse_iso_duration
)

# Use the NEW key you just created (not the leaked one!)
API_KEY = "AIzaSyBWK53sduwkSKqUmEFK9QMkkxFMYyEgnK0"
client = get_client(API_KEY)

test_url = "https://youtu.be/U2SVCCENLjE?si=Ev5Tp6d8tUsAR9Cj"
video_id = parse_video_id(test_url)
print(f"Video ID: {video_id}")

video = fetch_video_meta(client, video_id)
print(f"\n📹 Video: {video.title}")
print(f"   Channel: {video.channel_title}")
print(f"   Views: {video.view_count:,}")
print(f"   Likes: {video.like_count:,}")
print(f"   Comments: {video.comment_count:,}")
print(f"   Duration: {parse_iso_duration(video.duration_iso)}")

channel = fetch_channel_info(client, video.channel_id)
print(f"\n📺 Channel: {channel.title}")
print(f"   Subscribers: {channel.subscriber_count:,}")
print(f"   Total videos: {channel.video_count:,}")
print(f"   Total views: {channel.view_count:,}")

rate = engagement_rate(video.view_count, video.like_count, video.comment_count)
print(f"\n💯 Engagement rate: {rate * 100:.3f}%")

comments = fetch_comments(client, video_id, max_total=5)
print(f"\n💬 Sample of {len(comments)} comments:")
for c in comments[:5]:
